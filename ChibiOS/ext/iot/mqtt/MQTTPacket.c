 ...
 */
int MQTTPacket_decode(int (*getcharfn)(unsigned char*, int, void*), int* value, void *u)
{
	unsigned char c;
	int multiplier = 1;
	int len = 0;
...
int bufchar(unsigned char* c, int count, void *u)
{
	int i;
	unsigned char* bufptr = (unsigned char*) u;
	for (i = 0; i < count; ++i)
		*c = *bufptr++;
	return count;
}
...
{
	return MQTTPacket_decode(bufchar, value, buf);
}
 ...
 */
int MQTTPacket_read(unsigned char* buf, int buflen, int (*getfn)(unsigned char*, int, void*), void *u)
{
	int rc = -1;
	MQTTHeader header = {0};
	int len = 0;
	...
	MQTTPacket_decode(getfn, &rem_len, u);
	len += MQTTPacket_encode(buf + 1, rem_len); /* put the original remaining length back into the buffer */

	/* 3. read the rest of the buffer using a callback to supply the rest of the data */
	if ((*getfn)(buf + len, rem_len, u) != rem_len)
		goto exit;
