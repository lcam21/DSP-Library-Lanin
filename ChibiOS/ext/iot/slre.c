...
#define __isxdigit(x) isxdigit((int)(x))
  ...
  while (len <= re_len && re[len] != ']' && result <= 0) {
    ...
    if (re[len] != '-' && re[len + 1] == '-' && re[len + 2] != ']' &&
      ...
      result = info->flags &&  SLRE_IGNORE_CASE ?
        _tolower(*s) >= _tolower(re[len]) &&
        _tolower(*s) <= _tolower(re[len + 2]) :
        *s >= re[len] && *s <= re[len + 2];
      len += 3;
    } else {
  ...
  for (i = 0; i < re_len; i += step) {
    ...
    if (re[i] == '|') {
      ...
      if (re[i + 1] == 'x') {
        ...
        FAIL_IF(re[i + 1] ==  'x' && !(__isxdigit(re[i + 2]) &&
                __isxdigit(re[i + 3])), SLRE_INVALID_METACHARACTER);
      } else {
